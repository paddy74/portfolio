...
Add the public SSH key to your account on the remote Git server. We recommend naming your key using your username and the name or ID of your client system, such as `jhdoe-AAAAA0000000000` or `AAAAA0000000000.auth.example.com`.
...
2. Select the `9th` option. This will create an ECC (elliptic curve cryptography) public/private keypair and an ECC signing key.
...
To stop Git from prompting for you password everytime you sign a commit or use `git push`, `git pull`, or `git fetch`, you can configure a credential helper to cache your credentials.

### SSH Agent Credential Caching

Use the `ssh-agent` to avoid entering your SSH passphrase every time you use `git push`, `git pull`, or `git fetch`, as described at [Configuring SSH Agent Caching](./setup-ssh-caching.mdx)

### GPG Agent Credential Caching

Use the `gpg-agent` to avoid entering your GPG passphrase every time you sign a commit, as described at [Configuring GPG Agent Caching](./setup-gpg-caching.md).

### Git Credential Cache (HTTPS)

Use the [Git Credential Cache](https://git-scm.com/doc/credential-helpers) to store credentials temporarily in memory or the system keyring. This is the simplest and most secure option for short-term use. However, it _only_ works for `https` and does not work for `ssh` or personal access tokens[^6].

[^6]: ["git-credential-cache is inherently unsuitable for persistent storage of personal access tokens."](https://git-scm.com/docs/git-credential-cache#_personal_access_tokens)
...
If you are using **Git for Windows** (not recommended), use:
